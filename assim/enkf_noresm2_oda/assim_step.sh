...
ODA () {
  ...
  do
    ...
    then
      ...
      # link file for vertical localsiation
      ln -sf $INPUTDATA_ASSIM/enkf/vert_loca/step-${MM}-ens$ENSSIZE.nc step.nc
  ...
  if [ ! -d ./RESULT/${YYYY}_${MM} ] ; then
    mkdir -p ./RESULT/${YYYY}_${MM}  || { echo "Could not create RESULT/${YYYY}_${MM} dir" ; exit 1 ; }
  fi
  mv enkf_diag_*.nc observations-*.nc observations.uf* tmpX5_*.uf ./RESULT/${YYYY}_${MM}/
