...
contains 
  ...
  !
  subroutine update_fields(ni, nj, nrens, nfields, nobs_array, depths, fld, infls,isdp,names,levels,step,issst)
#if defined (QMPI)
    use qmpi
#else
    use qmpi_fake
    ...
    character(len=8), dimension(nfields), intent(in) :: names
    integer, dimension(nfields), intent(in) :: levels
    integer, dimension(ni, nj), intent(in) :: step
    logical, intent(in) :: issst ! whether the current observation is SST
...
11                    n = 0
             ...
             do f = 1, nfields
                if (issst) then
                   if ( (trim(names(f)) .eq. 'temp' .or. trim(names(f)) .eq. 'saln') .and. (levels(f) .gt. step(i,j)) ) cycle
                end if
                infl = infls(f) ! conversion to real(4)
                if (infl /= 1.0) then
                   IM = - (infl - 1.0) / real(nrens, 4)
    ...
    do o = 1, nobs
       if (trim(obs(o) % id) .eq. 'SST') then
          ...
          if (dist(o) >= rmax) then
             ...
             lobs(ngood) = o
             nsst = 1
          else if (dist(o) < dist(lobs(ngood))) then
             lobs(ngood) = o
          end if
       end if
    end do
