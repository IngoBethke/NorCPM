...
!
! Modifications:
!                12/08/2022 TS&YW: Small changes to allow to read/prepare
!                obsdata only on the root process and broadcast to the other processes, 
!                making the code more efficient (old version: EnKF.F90.12.08.2022)
...
program EnKF
  ...
  if (master) then
     ...
     ! get vertical localisation level
     !
     call get_micom_step(step, idm, jdm)
  end if

#if defined(QMPI)
  call barrier()
  call mbroadcast(meandx,mindx)
  call broadcast(modlon)
  call broadcast(modlat)
  call broadcast(depths)
  call broadcast(step)
#endif
  ...
  if (master) then
     ...
     N=size(transfer(obs,buffer))   !! compute size
  end if

#if defined(QMPI)
  call barrier()
  call mbroadcast(nobs,nuobs,N)
  call broadcast(unique_obs)
  call broadcast(nobseach)
  call broadcast(uobs_begin)
  call broadcast(uobs_end)
  call broadcast(issst)
#endif

  allocate(buffer(N))
  if (master) then
     buffer = transfer(obs,buffer)
  end if
...
!              rtc() - time2
     ...
     call update_fields(idm, jdm, ENSSIZE, nfields, nlobs_array, depths,&
          fld(1,1), infls, isdp, fieldnames(m1:m2), fieldlevel(m1:m2), step, issst)
