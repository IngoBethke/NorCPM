...
!  use m_read_MET_SST_grid
  ...
  if (trim(producer) == 'Reynolds') then
     ...
     if (trim(obstype) == 'SST') then
        ...
        call read_MET_SST(fname,fnamehdr,data,modlon, modlat, depths,360,180, nrobs)
        allocate(obs(nrobs))
        data_eq_obs = .false.
        print *,'MET Data read',nrobs
   ...
   else if (trim(Producer) == 'blom') then
     if (trim(obstype) == 'SST') then
        dosuperob = .false.
        mask(:,:)=0
        where(depths>0) mask=1;
        ...
        allocate(data(grpoints))
       !fnamehdr contains the month and fname the year file
        call read_blom_SST(fname,fnamehdr,data,modlon, modlat, depths, nx, ny, nrobs)
        data_eq_obs = .true.
        print *,'Data read'
      endif
  ...
  elseif (trim(producer) == 'EN4') then
     ...
     data_eq_obs = .true.
  elseif (trim(producer) == 'CODC') then

     dosuperob = .true.
     is3d = .true.
     read(fnamehdr, *) var
     call read_CODC_profile(fname, obstype, var, nx, ny, data, nrobs)
