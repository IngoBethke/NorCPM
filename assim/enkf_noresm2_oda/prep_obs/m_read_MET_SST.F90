...
contains
  ...
  subroutine read_MET_SST(fname,cens,data,modlon,modlat,depths,dlon,dlat,nrobs)
  use mod_measurement
  use mod_grid
  use nfw_mod
...
!Read the monthly mean 
  ...
  do j = 1, dlat
     do i = 1, dlon
          if (vsic(i,j,1).eq.0. .and. verr(i,j,1).gt.0.) then
           nrobs=nrobs+1
           ...
           data(nrobs)%var = max(real(verr(i,j,1)*3)**2,0.01) ! apply multiplication factor 3 to match HadISST2 error
           !data(nrobs)%var = max(real(verr(i,j,1)*6)**2,0.01) ! apply multiplication factor 6 
          endif
     enddo   ! dlat
  enddo    ! dlon
